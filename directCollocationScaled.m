function soln = directCollocationScaled(problem)
...
% These bounds are what we scale all the scalable variables to.
SCALED_UB = 2.0;
SCALED_LB = 1.0;
...
% Logical array that tells us whether the variables are scalable.
% Unscalable variables have inf bounds.
scalable = ~(isinf(zLow) | isinf(zUpp));

% Create the scaler for the bounds.
% znew = m z + b
% znew_2 - znew_1 = m(z_2 - z_1)
% m = (znew_2 - znew_1)/(z_2 - z_1)
%
% b = znew_2 - m*z_2
%
% m z = znew - b
...
scalable(abs(range) < 1e-12) = false;

m = (SCALED_UB - SCALED_LB)./range;
b = (SCALED_UB - m.*zUpp);

binv = -b./m;
minv = 1./m;

scaler.fscalez = @(z) scaleFunction(z, m, b, scalable);
scaler.funscalez = @(z)  scaleFunction(z, minv, binv, scalable);
problem.scaler = scaler;

scale_test_lb = (abs(SCALED_LB - scaler.fscalez(zLow)) > 1e-15) & scalable;
scale_test_ub = (abs(SCALED_UB - scaler.fscalez(zUpp)) > 1e-15) & scalable;
assert(all(~scale_test_lb));
assert(all(~scale_test_ub));

scale_test_lb_inv = (abs(zLow - scaler.funscalez(SCALED_LB*ones(size(zLow)))) > 1e-15) & scalable;
scale_test_ub_inv = (abs(zUpp - scaler.funscalez(SCALED_UB*ones(size(zUpp)))) > 1e-15) & scalable;
assert(all(~scale_test_lb_inv))
assert(all(~scale_test_ub_inv));

%%%% Set up problem for fmincon:
if flagGradObj
    P.objective = @(z)( ...
        myObjGrad(scaler.funscalez(z), pack, F.pathObj, F.bndObj, F.weights, gradInfo) );   %Analytic gradients
    [~, objGradInit] = P.objective(zGuess);
    sparsityPattern.objective = (objGradInit~=0)';  % Only used for visualization!  
else
    P.objective = @(z)( ...
        myObjective(scaler.funscalez(z), pack, F.pathObj, F.bndObj, F.weights) );   %Numerical gradients
end
if flagGradCst
    P.nonlcon = @(z)( ...
        myCstGrad(scaler.funscalez(z), pack, F.dynamics, F.pathCst, F.bndCst, F.defectCst, gradInfo) ); %Analytic gradients
    ...
    P.nonlcon = @(z)( ...
        myConstraint(scaler.funscalez(z), pack, F.dynamics, F.pathCst, F.bndCst, F.defectCst) ); %Numerical gradients
end


P.x0 = scaler.fscalez(zGuess);
P.lb = scaler.fscalez(zLow);
P.ub = scaler.fscalez(zUpp);
P.Aineq = []; P.bineq = [];
P.Aeq = []; P.beq = [];
P.options = Opt.nlpOpt;
P.options.OutputFcn = @(x,optimValues,state)logOptSteps(x,optimValues,pack,scaler);
P.solver = 'fmincon';

%%%% Call fmincon to solve the non-linear program (NLP)
tic;
[zSoln, objVal,exitFlag,output] = fmincon(P);
[tSoln,xSoln,uSoln] = unPackDecVar(scaler.funscalez(zSoln),pack);
nlpTime = toc;
...
function z = scaleFunction(z, m, b, scalable)

z(scalable) = m(scalable).*z(scalable) + b(scalable);
